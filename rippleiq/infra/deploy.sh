...
RESOURCE_GROUP="${RESOURCE_GROUP:-rippleiq-rg-$SUFFIX}"
LOCATION="${LOCATION:-swedencentral}"
FOUNDRY_RESOURCE_NAME="${FOUNDRY_RESOURCE_NAME:-rippleiq-$SUFFIX}"
PROJECT_NAME="${PROJECT_NAME:-rippleiq-project}"
MODEL_DEPLOYMENT_NAME="${MODEL_DEPLOYMENT_NAME:-gpt-5.4}"
MODEL_NAME="${MODEL_NAME:-gpt-5.4}"
MODEL_VERSION="${MODEL_VERSION:-2026-03-05}"
MINI_DEPLOYMENT_NAME="${MINI_DEPLOYMENT_NAME:-gpt-5.4-mini}"
LOG_ANALYTICS_NAME="${LOG_ANALYTICS_NAME:-rippleiq-logs-$SUFFIX}"
APP_INSIGHTS_NAME="${APP_INSIGHTS_NAME:-rippleiq-insights-$SUFFIX}"
...
MINI_VERSION=$(az cognitiveservices account list-models \
    --name "$FOUNDRY_RESOURCE_NAME" \
    --resource-group "$RESOURCE_GROUP" \
    --query "[?name=='$MINI_DEPLOYMENT_NAME'].version | sort(@) | [-1]" -o tsv)

echo ">>> Deploying model: $MINI_DEPLOYMENT_NAME ($MINI_VERSION)..."
az cognitiveservices account deployment create \
    --name "$FOUNDRY_RESOURCE_NAME" \
    --resource-group "$RESOURCE_GROUP" \
    --deployment-name "$MINI_DEPLOYMENT_NAME" \
    --model-name "$MINI_DEPLOYMENT_NAME" \
    --model-version "$MINI_VERSION" \
    --model-format OpenAI \
    --sku-capacity 50 \
    --sku-name GlobalStandard \
    --output none
